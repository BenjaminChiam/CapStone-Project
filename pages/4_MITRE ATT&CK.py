    ...
    if selected_tech_id:
        ...
        if tech:
            ...
            for tid in tech.tactic_ids:
                for t in TACTICS:
                    if t.id == tid:
                        tactic_names_list.append(f"{t.name} ({t.id})")
            st.markdown(f"**Tactics:** {', '.join(tactic_names_list)}")
...
# ════════════════════════════════════════════════════════════════════
# TAB 4: Detection Coverage Tracker (FIXED — no duplicate keys)
# ════════════════════════════════════════════════════════════════════
with tab4:
    ...
    # Get all unique parent techniques (deduplicated)
    all_parent_techs = [t for t in TECHNIQUES if not t.is_subtechnique]
    # Deduplicate by technique ID (some techniques map to multiple tactics)
    seen_ids = set()
    unique_parent_techs = []
    for t in all_parent_techs:
        if t.id not in seen_ids:
            unique_parent_techs.append(t)
            seen_ids.add(t.id)
    ...
    with col_q1:
        if st.button("Select All", use_container_width=True):
            st.session_state.covered_techniques = {t.id for t in unique_parent_techs}
            st.rerun()
    with col_q2:
    ...
    # Show techniques grouped by PRIMARY tactic, but each technique only once
    displayed_ids = set()
    for tactic in sorted(TACTICS, key=lambda t: t.order):
        techs_in_tactic = get_parent_techniques_by_tactic(tactic.id)
        # Filter out already-displayed techniques
        new_techs = [t for t in techs_in_tactic if t.id not in displayed_ids]
        if not new_techs:
            continue
        with st.expander(f"{tactic.name} ({len(new_techs)} techniques)", expanded=False):
            for tech in new_techs:
                displayed_ids.add(tech.id)
                checked = tech.id in st.session_state.covered_techniques
                # Key is now guaranteed unique since each tech.id only appears once
    ...
    # Coverage by tactic (a technique counted under each tactic it belongs to)
    cov_data = []
    for tactic in sorted(TACTICS, key=lambda t: t.order):
        techs = get_parent_techniques_by_tactic(tactic.id)
        # Deduplicate within this tactic
        tactic_ids = list({t.id for t in techs})
        total_in_tactic = len(tactic_ids)
        covered_in_tactic = sum(1 for tid in tactic_ids if tid in st.session_state.covered_techniques)
