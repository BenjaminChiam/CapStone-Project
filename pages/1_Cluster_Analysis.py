...
and DGA domain scanning. Results persist in session_state for AI follow-up.
"""
...
with tab1:
    ...
    if st.button("🔬 Run Cluster Analysis", use_container_width=True):
        ...
        if len(iocs) < 4:
            ...
            st.session_state["cluster_results"] = df
...
with tab3:
    ...
    if st.button("🧬 Scan for DGA Patterns", use_container_width=True):
        ...
        st.session_state["dga_results"] = pd.DataFrame(results)

    # Render from session state
    if "dga_results" in st.session_state:
        df = st.session_state["dga_results"]
